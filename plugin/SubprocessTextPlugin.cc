...
private:
    void fillComponents(ast::TreePtr &constant) {
        while (constant) {
            if (auto unresolved = ast::cast_tree_const<ast::UnresolvedConstantLit>(constant)) {
                components.push_back(unresolved->cnst);
                constant = unresolved->scope.get();
            } else if (auto ident = ast::cast_tree_const<ast::UnresolvedIdent>(constant)) {
                ENFORCE(ident->name == core::Names::singleton());
                components.push_back(core::Names::singleton());
                break;
            } else if (auto constLit = ast::cast_tree_const<ast::ConstantLit>(constant)) {
...
pair<ast::TreePtr, vector<shared_ptr<core::File>>>
SubprocessTextPlugin::run(core::Context ctx, ast::TreePtr tree) {
